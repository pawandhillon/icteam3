...
function formbuilder_checkCreditCard ($cardnumber, $cardname='', &$errornumber, &$errortext) {
  ...
  // Clean the card number to ensure it only contains digits.
  $cardnumber = preg_replace('/[^\d]/', '', $cardnumber);
  
	// If cardname is not provided, attempt to detect card type based on criterion listed above.
	if(!$cardname)
	{
		$cardname = formbuilder_find_card_type($cardnumber, $cards);
	}
 ...
 * @return unknown_type
 */
function formbuilder_find_card_type($cardnumber, $cards)
{
	foreach($cards as $card)
	{
		$possible_lengths = explode(',', $card['length']);
		$possible_prefixes = explode(',', $card['prefixes']);
		
		// Length test
		foreach($possible_lengths as $length)
		{
			$length = trim($length);
			if(strlen($cardnumber) == $length)
			{
				// We found a matching length... let's test to see if the prefixes match.
