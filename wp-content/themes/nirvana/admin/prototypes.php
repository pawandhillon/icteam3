...
function cryout_proto_font($fonts,$sizes,$size,$font,$gfont,$labelsize,$labelfont,$labelgfont,$general=""){ ?>
	<?php if ($size>0): ?>
...
function cryout_proto_field($settings,$type,$name,$values,$labels='',$cls='',$echo=true){
	...
	switch ($type):
		case "checkbox": 
			$data = "<input name='${settings['id']}[$name]' type='hidden' value='0' /><input value='1' id='$name' name='${settings['id']}[$name]' type='checkbox' ".checked($values,'1',false). " class='$cls'/> ".
